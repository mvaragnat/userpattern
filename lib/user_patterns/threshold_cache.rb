...
module UserPatterns
  ...
  class ThresholdCache
    ...
    def build_limits
      multiplier = UserPatterns.configuration.threshold_multiplier
    ...
    def safe_refresh
      ...
      # :nocov:
      Rails.logger&.error("[UserPatterns] Threshold refresh error: #{e.message}")
      # :nocov:
    end
    ...
    def start_refresh_timer
      @timer = Concurrent::TimerTask.new(
        execution_interval: UserPatterns.configuration.threshold_refresh_interval
      ) { safe_refresh }
      @timer.execute
    end
    # :nocov:
