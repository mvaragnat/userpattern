...
RSpec.describe UserPatterns::Anonymizer do
  ...
  describe '.anonymize' do
    ...
    it 'varies when the salt changes' do
      first = described_class.anonymize(request)
      UserPatterns.configuration.anonymous_salt = 'completely_different_salt_value!!'
      second = described_class.anonymize(request)
  ...
  describe 'configured detection modes' do
    it ':session always uses the session id' do
      UserPatterns.configuration.session_detection = :session
    ...
    it ':header always uses the Authorization header' do
      UserPatterns.configuration.session_detection = :header
    ...
    it 'accepts a custom Proc' do
      UserPatterns.configuration.session_detection = ->(req) { "custom_#{req.remote_ip}" }
    ...
    it 'raises a TypeError for an unknown detection mode' do
      UserPatterns.configuration.session_detection = :unknown_mode
